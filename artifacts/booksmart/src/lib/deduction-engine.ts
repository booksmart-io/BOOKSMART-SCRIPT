...
export type OrgRow = Record<string, unknown> & {
  id?: number;
  state?: number | null;
};
...
export function getApplicableRule(
  ...
  const candidateGroups = groups
    ...
    .filter((g) =>
      jurisdiction === "federal"
        ? g.state_id === null
        : orgStateId != null && g.state_id === orgStateId,
    )
    .sort(
      (a, b) =>
  ...
  for (const g of candidateGroups) {
    const rule = rules.find(
      (r) =>
        r.deduction_rule_group_id === g.id &&
...
function ruleAmountForTx(
  ...
  if (rule.calculation_type === "percentage") {
    ...
    return rule.max_deduction_per_transaction != null
      ? Math.min(amt, rule.max_deduction_per_transaction)
      : amt;
  }
  // fixed
  return rule.max_deduction_per_transaction != null
...
export function summarizeDeductions(
  ...
  for (const tx of txs) {
    ...
    const transactionDate = tx.date_time ? new Date(tx.date_time) : asOf;
    const effectiveDate = Number.isNaN(transactionDate.getTime())
      ? asOf
      : transactionDate;
    const federalRule =
    ...
    if (!federalRule) {
      ...
      !federalRule.is_per_transaction &&
      federalRule.calculation_type === "fixed"
    ) {
      federalAmt = appliedOnceFederal.has(federalRule.id)
        ? 0
        : ruleAmountForTx(federalRule, base, org);
    ...
    if (!stateRule) {
      ...
      !stateRule.is_per_transaction &&
      stateRule.calculation_type === "fixed"
    ) {
      stateAmt = appliedOnceState.has(stateRule.id)
        ? 0
        : ruleAmountForTx(stateRule, base, org);
