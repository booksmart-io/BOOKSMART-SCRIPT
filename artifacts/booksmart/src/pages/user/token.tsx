...
function StoreCard({
  ...
  return (
    <div className="relative flex min-h-[176px] flex-col rounded-lg border border-border bg-card p-5 text-card-foreground shadow-sm">
      {item.rewardText && <div className="absolute right-5 top-5 text-sm font-bold text-primary">{item.rewardText}</div>}
      <div className="flex items-start gap-4">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
          <Icon className="h-6 w-6" />
        </div>
        <div className="min-w-0 pr-8">
          <h3 className="text-base font-bold leading-tight text-card-foreground">{item.title}</h3>
      ...
      <div className="mt-auto flex items-end justify-between gap-4 pt-8">
        <div className="flex items-center gap-2 text-base font-bold text-card-foreground">
          <Coins className="h-4 w-4 fill-primary text-primary" />
          {item.tokens} Tokens
        </div>
        <Button
          type="button"
          variant="outline"
          disabled
          className="h-10 min-w-[154px] rounded-lg border-primary/40 bg-transparent px-5 text-sm font-extrabold text-primary/60 opacity-70"
        >
          {item.button}
        </Button>
      </div>
  ...
  return (
    <div className="min-h-full bg-background px-3 py-5 text-foreground sm:px-6 sm:py-7">
      <div className="mb-6 flex flex-wrap justify-start gap-3 sm:mb-10 sm:justify-end sm:gap-4">
        <button className="flex h-10 items-center gap-2 rounded-full border border-border bg-card px-4 text-base font-bold text-card-foreground">
      ...
      <div className="grid min-w-0 gap-8 xl:grid-cols-[minmax(0,1fr)_414px]">
        <main className="space-y-8">
          <section className="flex min-h-[130px] flex-col items-start gap-4 rounded-lg border border-border bg-card px-5 py-5 text-card-foreground sm:flex-row sm:items-center sm:px-8 sm:py-0">
            <div className="flex h-[64px] w-[64px] shrink-0 items-center justify-center rounded-full bg-primary/15 text-primary sm:h-[74px] sm:w-[74px]">
            ...
            <div className="min-w-0 sm:ml-7">
              ...
              <div className="mt-1 flex items-center gap-2 text-xl font-bold text-card-foreground">
          ...
          <section>
            ...
            <div className="flex h-[72px] items-center justify-center rounded-lg border border-border bg-card text-base font-semibold text-muted-foreground">
        ...
        <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start">
          <section className="rounded-lg border border-border bg-card p-6 text-card-foreground">
                ...
                return (
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-4">
                      ...
                      <div className="text-sm font-extrabold text-primary">+ {PACKAGE_BONUSES[pkg.key]} Tokens</div>
                    </div>
                    <div className="mb-2 text-base font-bold">{money(pkg.unitAmount)}</div>
                    <div className="h-1.5 rounded-full bg-muted">
                      <div
                        className={`h-full rounded-full ${active ? "bg-primary" : "bg-primary/80"}`}
                        style={{ width: `${percent}%` }}
                      />
                    </div>
                  </button>
            ...
            <Button
              ...
              onClick={() => handleBuy()}
              className="mt-9 h-14 w-full rounded-lg border border-primary bg-transparent text-lg font-extrabold text-primary hover:bg-primary hover:text-primary-foreground"
            >
              {loadingPackage ? (
          ...
          <section className="rounded-lg border border-border bg-card p-6 text-card-foreground">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button className="text-sm font-bold text-card-foreground">View More &gt;</button>
            </div>
            {!history.length ? (
              ...
              <div className="space-y-5">
                {history.slice(0, 4).map((tx) => (
                  <div key={tx.id} className="grid grid-cols-[70px_minmax(0,1fr)_auto] items-center gap-3 text-sm">
                    ...
                    <div className="truncate font-bold text-card-foreground">{historyLabel(tx)}</div>
            ...
            {history.length > 4 && (
              <button className="mt-8 flex w-full items-center justify-center gap-2 text-base font-bold text-card-foreground">
                View More
          ...
          <section className="rounded-lg border border-border bg-card p-6 text-card-foreground">
            ...
            <div className="space-y-5">
              ...
              {!history.length && <p className="text-sm font-medium text-muted-foreground">Activity will appear after purchases or unlocks.</p>}
            </div>
          </section>
        </aside>
      </div>
