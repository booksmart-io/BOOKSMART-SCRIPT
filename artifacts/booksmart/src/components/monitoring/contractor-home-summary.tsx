...
import { loadContractorIntelligence, type TrustedMetric } from "@/lib/contractor-intelligence-client";
...
export function ContractorHomeSummary({ organizationId, start, end, healthScoreSlot }: { organizationId: number; start: Date; end: Date; healthScoreSlot?: ReactNode }) {
  ...
  const receivablesRoute = firstOverdueInvoice
    ? `/user/jobber-records?object_type=invoices&record_id=${encodeURIComponent(firstOverdueInvoice.id)}`
    : "/user/money#receivables";
  const attentionItems = [
    ...(data.accountsReceivable.overdueAmount.value && data.accountsReceivable.overdueAmount.value > 0 ? [{ key: "overdue", icon: Clock3, title: `${money.format(data.accountsReceivable.overdueAmount.value)} overdue`, detail: `${data.accountsReceivable.overdueInvoiceCount} invoice${data.accountsReceivable.overdueInvoiceCount === 1 ? "" : "s"} need attention`, route: receivablesRoute, action: "Review receivables" }] : []),
  ...
  return <section className="space-y-4" aria-label="Contractor financial intelligence">
    ...
    <div className={healthScoreSlot ? "grid items-stretch gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(18rem,1fr)]" : undefined}>
      {attentionItems.length > 0 ? <Card className="h-full border-amber-400/30"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2"><CircleAlert className="h-5 w-5 text-amber-400" />Immediate attention</CardTitle></CardHeader><CardContent className="space-y-2">{attentionItems.map(item => <div key={item.key} className="flex flex-col gap-3 rounded-lg border bg-background/20 p-4 sm:flex-row sm:items-center"><div className="flex min-w-0 gap-3 sm:contents"><item.icon className="mt-0.5 h-5 w-5 shrink-0 text-amber-400" /><div className="min-w-0 flex-1"><p className="font-medium">{item.title}</p><p className="text-xs text-muted-foreground">{item.detail}</p></div></div><Button asChild size="sm" variant="outline" className="w-full sm:w-fit"><Link href={item.route}>{item.action}</Link></Button></div>)}</CardContent></Card> : <div className="flex h-full items-start gap-3 rounded-lg border p-4 sm:items-center"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" /><div><p className="font-medium">Nothing needs immediate attention</p><p className="text-sm text-muted-foreground">No overdue receivables, pending job warnings, or unverified cash issues were found.</p></div></div>}
      {healthScoreSlot}
    </div>
...
export function RecentAccountActivity() {
  ...
  return <Card className="flex h-full min-h-0 flex-col xl:h-[30rem]"><CardHeader className="flex-row items-center justify-between gap-3"><CardTitle>Recent account activity</CardTitle><span className="text-xs text-muted-foreground">Latest 3</span></CardHeader><CardContent className="min-h-0 flex-1 space-y-2 overflow-y-auto">{activity.isLoading ? <div className="flex items-center gap-2 p-3 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading activity…</div> : recentActivity.length === 0 ? <Empty text="New transactions, job matches, and task updates will appear here." /> : recentActivity.map(item => <ActivityRow key={item.id} item={item} compact />)}</CardContent></Card>;
}

function ActivityRow({ item, compact = false }: { item: ActivityNotification; compact?: boolean }) {
  return <Link href={item.route || "/user/money"} className="flex items-center gap-3 rounded-lg border p-3 transition-colors hover:bg-muted/40"><div className={`h-2 w-2 shrink-0 rounded-full ${item.read_at ? "bg-muted-foreground/40" : "bg-primary"}`} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.title}</p><p className="line-clamp-2 text-xs text-muted-foreground">{item.description}</p>{compact && <p className="mt-1 text-[11px] text-muted-foreground">{new Date(item.created_at).toLocaleDateString()}</p>}</div>{!compact && <span className="hidden shrink-0 text-xs text-muted-foreground sm:block">{new Date(item.created_at).toLocaleDateString()}</span>}<ArrowRight className="h-4 w-4 shrink-0 text-muted-foreground" /></Link>;
}
