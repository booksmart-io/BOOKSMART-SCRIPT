-- Correct the removal lifecycle: the accounting transaction still exists, so
-- removing only its job assignment must return it to review rather than
-- permanently suppressing it as rejected.
BEGIN;

UPDATE public.contractor_financial_matches m SET
  status = 'suggested', confidence = 'high', requires_confirmation = true,
  updated_at = now()
WHERE m.source_record_type = 'transaction'
  AND m.status = 'rejected'
  AND m.removed_at IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.contractor_job_cost_assignments a
     WHERE a.organization_id = m.organization_id
       AND a.source_provider = m.source_provider
