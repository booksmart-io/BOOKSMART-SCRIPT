...
async function plaidFetch<T>(path: string, body: Record<string, unknown>): Promise<T> {
  ...
  if (!response.ok) {
    const message = json?.error_message ?? json?.error_code ?? "Plaid request failed";
...
router.post("/plaid/link-token", requireAuth, async (req, res) => {
  try {
    ...
    const data = await plaidFetch<{
      ...
      products: envList("PLAID_PRODUCTS", "transactions"),
    ...
    res.status(500).json({ error: "plaid_link_token_failed", message: err instanceof Error ? err.message : "Plaid Link token failed" });
  }
});
...
router.post("/plaid/exchange-public-token", requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "plaid_exchange_failed", message: err instanceof Error ? err.message : "Plaid exchange failed" });
  }
});
...
router.post("/plaid/sync", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true, added: addedCount, modified: modifiedCount, removed: removedCount });
  } catch (err) {
    console.error("[plaid/sync]", err);
    res.status(500).json({ error: "plaid_sync_failed", message: err instanceof Error ? err.message : "Plaid sync failed" });
  }
});
